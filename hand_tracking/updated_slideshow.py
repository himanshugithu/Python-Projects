...
def main():
    ...
    prev_index_middle_fingers_state = None
    ...
    while True:
        ...
        if len(lmList) != 0 and detector.handedness:
            ...
            if handedness == "Left":
                ...
                if index_middle_fingers_state == [1, 1]:
                    if not finger_up:
                        finger_up = True
                        prev_index_finger_pos = index_finger_pos
                        action_triggered = False
                    elif not action_triggered and prev_index_finger_pos:
                        dx = index_finger_pos[1] - prev_index_finger_pos[1]
                        if abs(dx) > movement_threshold:
                            if dx > 0:
                                print("Change to next slide")
                                pyautogui.press('right')
                                print('right')
                                time.sleep(1)
                            else:
                                print("Change to previous slide")
